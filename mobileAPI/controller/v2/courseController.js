...
var course = require( __mobileAPIBase + 'module/v2/course');
...
module.exports.getGrade = function(req, res, next) {

    /**
     * Check params is exists
     */
    if (!req.body.token || !req.params.type || !req.body.id) {
      res.status(400).json({
        statusCode: 1101,
        status: 'Params illegal'
      });
    ...
    if (!Number.isInteger(req.body.id) && !(req.body.id instanceof Array)) {
      res.status(400).json({
        statusCode: 1101,
        status: 'Params illegal'
      });
    ...
    if (req.body.id instanceof Array) {
      req.body.id = req.body.id.filter((cv) => (
        Number.isInteger(cv) && cv > 0
      ))
      if (req.body.id.lenth === 0) {
        res.status(400).json({
          statusCode: 1101,
          status: 'Params illegal'
        });
    ...
    } else if (Number.isInteger(req.body.id) && req.body.id < 0) {
      res.status(400).json({
        statusCode: 1101,
        status: 'Params illegal'
      });
     ...
     * Verify token
     */
    verifyToken(req.body.token, lookupDB, res)

    /**
     * look up data base
     */

    function lookupDB(userData) {
      var task = (() => {
        switch(req.params.type) {
          case "homework": {
            var homework_id = req.body.id instanceof Array ? req.body.id : [req.body.id]
            return course.homeworkGrade({
              user_uid: userData.uid,
      ...
      task.then(
        ...
        (rejectTask) => {
          ...
          console.error(rejectTask.err, rejectTask.query.sql)
        }
      )
    }
}
